...
export default function ReviewForm() {
  ...
  const buildFormData = (data: FormData, status: "PENDING" | "DRAFT") => {
    // console.log(data);
    const formData = new FormData();
  ...
  const onSubmit = async (data: FormData) => {
    setIsSubmitting(true);
    try {
      const res = await createNormalReview(buildFormData(data, "PENDING"));
  ...
  const saveDraft = async (data: FormData) => {
    setIsDrafting(true);
    try {
      const res = await createNormalReview(buildFormData(data, "DRAFT"));
      ...
      <CardContent className="p-4 md:p-6">
        ...
        >
          {({ form }) => (
            <>
              {/* IMAGE UPLOAD */}
              <CImageUpload label="Product Images" name="images" />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 2xl:gap-10">
                ...
                <CRating required name="rating" label="Rating" />
              </div>
